...
function initPage(){
	...
	members = common.getUrlParams("members");
	if(!isOnline){
	...
	widget.bindTouchClick($(".qrcode"),function(e){
		$('.qrcode-img').qrcode({
		    ...
		    width: $('.qrcode-img').width(), //宽度 
		    height:$('.qrcode-img').height(), //高度 
...
function redirect(){
	var url = null;
	if(isOnline){
		url = "stat.html?is_super="+(isSuper()?1:0)+"&order_by="+orderBy+"&is_online=1&start_date="+startDate+"&end_date="+endDate;
	}else{
		url = "stat.html?is_super="+(isSuper()?1:0)+"&order_by="+orderBy+"&is_online=0&start_month="+startMonth+"&end_month="+endMonth;
	}
	url += '&members='+(members||"");
	common.gotoPage(url);
}
...
function loadData(orderBy){
	var url = orderBy=='realtime' || orderBy=='daily' || orderBy=='line' ?
			"/web/shanghui/queryFightingList.do"
	...
	ajax.request({
		...
		params : {
			...
			is_order_by_date_desc : orderBy=='line' ?0 : 1	
		...
		success : function(header,body){
			...
			if(orderBy=='realtime' || orderBy=='daily'){
				...
				drawLine(fightingList);
			}else{
				//huifei = body.huifei;
				//shui = body.shui;
				drawSumCountBar(fightingList,orderBy);
...
function drawSumCountBar(fightingList,orderBy){
	$(".realtime-list").hide();
	var $statChart = $(".stat-total-chart");
	$statChart.show();
	...
	for(var key in membernames){
		if(key == "shui" || key == "huifei" || membernames[key].sum_stat === 0){
			continue;
		}
...
function drawLine(fightingList){
	$(".realtime-list").hide();
	$(".stat-total-chart").show();
	
	var $memberchoose = $(".stat-total-chart .member-choose");
	initMembChoose($memberchoose,members && members.split(","),function(membs,e){
		if(membs.length > 3){
			message.errorHide("最多只能选3个");
			return;
		}
		drawLineDate(fightingList,membs);
	});
	
	if($memberchoose.find(".common-bg").length > 0){
		drawLineDate(fightingList,members.split(","));
	}
}

function drawLineDate(fightingList,membs){
	var $statChart = $(".chat-container");
	$statChart.height(300);
	...
	var labels = [];
	var dayCount = 7;
	var membDaySum = [];//{name:吴宇杰,memb:wuyj,day:}
	var lastdate = null;
	var startIndex = fightingList.length > dayCount ? fightingList.length-dayCount : 0;
	for(var i=fightingList.length-dayCount;i<fightingList.length;i++){
		var ft = fightingList[i];
		var ftmembers = ft.members;
		var date = ft.date;
		labels.push(date.toDate().toStr("dd/MM"));
		
		
		for(var k=0;k<membs.length;k++){
			var memb = membs[k];
			var dayScore = ftmembers[memb] || 0;
			var seriesData = common.matchItem(memb,seriesDatas,"memb");
			if(seriesData == null){
				seriesDatas.push({
					memb:memb,
					name :  membernames[memb].name,
					data : [dayScore]
				});
			}else{
				seriesData.data.push(seriesData.data[seriesData.data.length-1]+dayScore);
			}
			/*var membScore = ftmembers[membs[k]];
			if(membDaySum[membs[k]] === undefined){
				membDaySum[membs[k]] = membScore;
			}else{
				membDaySum[membs[k]] += membScore;
			}*/
		}
	...
	/*seriesData = [{
        name: '吴宇杰',
        data: [100, 200, 190, 900, 500, -200]
    }, {
        name: '张宇',
        data: [300, 500, 290, 100, 200, -400]
    }];*/
	Highcharts.chart($statChart.get(0), {
        chart: {
            type: 'line'
	...
	for(var i=0;i<awardList.length;i++){
		...
		if(!award.members){
			...
			for(var k=0;k<award.members.length;k++){
				...
				var yvalue = null;
				if(award.name == "牺牲奖" || award.name == "发财奖"){
					yvalue = fting.sum;
				}else{
					yvalue = fting.count;
				}
...
function drawRealtime(fightingList,orderBy){
	...
	var $memberchoose = $(".realtime-list .member-choose");
	initMembChoose($memberchoose,true,function(e){
...
function initMembChoose($memberchoose,defaultChoosed,eventCallback){
	if($memberchoose.children().length > 0){
		return;
	}
	if(defaultChoosed && typeof defaultChoosed == 'object'){
		defaultChoosed = defaultChoosed.join(",");
	}
	var index = 1;
	for(var key in membernames){
		var membInfo = membernames[key];
		...
		var $span = $("<div>").appendTo($memberchoose).attr("member",key);
		$span.text(membInfo.name.substr(0,2)).addClass("item");
		if(index < 10 && membInfo.realtime_stat !== 0 && (defaultChoosed === true || (typeof defaultChoosed == 'string' && defaultChoosed.indexOf(key) > -1))){
			$span.addClass("common-bg");
		}
		index++;
	}
	...
	widget.bindTouchClick($items,function(e){
		...
		for(var i=0;i<$members.length;i++){
			membs.push($members.eq(i).attr("member"));
		}
		members = membs.join(',');
		eventCallback && eventCallback.apply(null,[membs,e]);
	});
}
	...
	for(var i=0;i<fightingList.length;i++){
		...
		widget.bindTouchClick($datetd,function(e){
			...
			if(orderBy == 'daily'){
				...
				common.gotoPage("stat.html?is_super="+(isSuper()?1:0)+"&is_online=1&start_date="+ft.date+"&end_date="+ft.date+"&order_by=realtime");
			}else{
				common.gotoPage("score.html?sid="+$tr.attr("fid")+"&is_super="+(isSuper()?1:0));
			}
