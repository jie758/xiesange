...
function queryDetail(){
	var isNew = groupbuyId == null;
	ajax.request({
		url : _base_url+"/web/manage/queryGroupbuyDetail.do",
		need_progressbar : false,
		params : {
			groupbuy_id : groupbuyId,
			need_wx_signature : !isNew
		},
		success : function(header,body){
			productList = body.productList;
			groupbuy = body.groupbuy;
			initIndex();
			if(!isNew){
				var loginUser = common.getLoginUser();
				var title = "#我要吃象山直运的纯美海鲜！"+loginUser.name+"邀请你来拼单#";
				var desc = "蟹三哥,地道的东海野生海鲜,拼单更优惠,快来和我一起拼海鲜吧~";
				initWxConfig(body.wxSignature,{
					url : buildTransitionUrl(_base_host+"/wechat/product/groupbuy.html?groupbuy_id="+groupbuyId),
					img : logoUrl,
					//timeline_desc : title+" "+desc,
					//message_title : title,
					//message_desc : desc,
					
					timeline_desc : "#蟹三哥# 快来和"+loginUser.name+"团购从象山直运的当日海鲜！鲜到崩溃！",
					message_title : "#蟹三哥# 快来和"+loginUser.name+"团购从象山直运的当日海鲜！",
...
function initIndex(){
	...
	widget.bindTouchClick($(".view-user-list"),function(e){
		common.gotoPage("../manage/m-groupbuy-join.html?groupbuy_id="+groupbuyId);
		e.stopPropagation();
	});
...
function initGroupbuy(){
	...
	$(".groupbuy-intro-input").val(groupbuy.intro);
	var chooseprodlist = groupbuy.productInfo.split(",");
	for(var i=0;i<chooseprodlist.length;i++){
		var stritems = chooseprodlist[i].split(":");
		var $proditem = $("#product_"+stritems[0]);
		$proditem.find(".groupbuy-price").val(stritems[1]);
		CheckboxSet.select($proditem.data("checkboxSet"),true);
	}
...
function refreshChooseProduct(){
	...
	var $choosecontent = $(".choose-prod-content");
	$choosecontent.empty();
	for(var i=0;i<$chooselist.length;i++){
		var $choose = $chooselist.eq(i);
		var $proditem = $choose.parents(".prod-item");
		var entity = $proditem.data("entity");
		var $div = div($choosecontent);
		$("<span>").appendTo($div).text(entity.name+(entity.spec && "("+entity.spec+")"));
		$("<span class='right-part'>").appendTo($div).text("￥"+$proditem.find(".groupbuy-price").val()+"/"+entity.unit)
	}
}
	...
	message.confirmSlide("确定提交?",function(){
		ajax.request({
			...
			success : function(header,body){
				...
				common.gotoPage("../manage/m-groupbuy-detail.html?groupbuy_id="+body.newid,2000);
			}
		});
	});
}
...
function initProdList(){
	...
	for(var i=0;i<productList.length;i++){
		...
		var $set = CheckboxSet.init($newDiv.find(".prod-choose"),{
			handler : function(selected){
				refreshChooseProduct();
			}
		});
		$newDiv.find(".groupbuy-price").val(prodEntity.price);
		$newDiv.find(".groupbuy-price").blur(function(){
			refreshChooseProduct();
		});
		$newDiv.data("checkboxSet",$set);
		$newDiv.show();
	}
}
