...
public class WechatNotifyTaskBean implements Runnable{
	...
	private WxTempMessageParam params;
	
	public WechatNotifyTaskBean(List<String> openidList,String tempid,String url,WxTempMessageParam params){
	...
	@Override
	public void run() {
		logger.debug("-------begin to run "+WechatNotifyTaskBean.class.getName());
		try {
			...
			for(int i=0;i<targets.size();i++){
				...
				try {
					...
					record.setChannel(BaseConsDefine.NOTIFY_CHANNEL.WECHAT.value());
		...
		logger.debug("-------finish "+WechatNotifyTaskBean.class.getName());
	}
}
