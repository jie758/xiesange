...
public class TaskManager {
    ...
    private static final DiscardPolicy HANDLER = new ThreadPoolExecutor.DiscardPolicy();    
    
    private static ThreadPoolExecutor THREAD_POOL;
    
    public static void init() {    
    	THREAD_POOL = new ThreadPoolExecutor(COREPOOLSIZE, MAXINUMPOOLSIZE, KEEPALIVETIME, UNIT, WORKQUEUE, HANDLER);    
        /*for (int i = 1; i < 11; i++) {    
            String task = "task@"+i;    
            ...
            threadPool.execute(new NotifyTaskBean(task));    
            try {    
                Thread.sleep(1000);    
    ...
    public static void execute(Runnable runnable,ICallback callback) {
    	THREAD_POOL.execute(runnable);
    }
}
